...
try:
    ...
    if creds_dict:
        ...
        df_lancamentos['DATA'] = pd.to_datetime(df_lancamentos['DATA'], format='%Y-%m-%d', errors='coerce')
        
        if df_lancamentos['DATA'].isna().all():
            df_lancamentos['DATA'] = pd.to_datetime(df_lancamentos['DATA'], format='%d/%m/%Y', errors='coerce')
        
        # Extrai MÊS e ANO
        df_lancamentos['MES_NUM'] = df_lancamentos['DATA'].dt.month
        df_lancamentos['ANO_NUM'] = df_lancamentos['DATA'].dt.year
        
        # Cria rótulo de mês
        meses_dict = {1: "JANEIRO", 2: "FEVEREIRO", 3: "MARÇO", 4: "ABRIL", 5: "MAIO", 6: "JUNHO",
                     7: "JULHO", 8: "AGOSTO", 9: "SETEMBRO", 10: "OUTUBRO", 11: "NOVEMBRO", 12: "DEZEMBRO"}
        
        df_lancamentos['MES_NOME'] = df_lancamentos['MES_NUM'].map(meses_dict)
        df_lancamentos['MES_REFERENCIA'] = df_lancamentos['MES_NOME'] + "/" + df_lancamentos['ANO_NUM'].astype(str)
        ...
        if len(df_entregador) > 0:
            ...
            # ===== EXIBIR MÉTRICAS =====
            st.subheader(f"📊 Resumo - {entregador}")
            ...
            st.subheader(f"📋 Lançamentos - {entregador}")
            ...
            # ===== RESUMO DE TODA A QUINZENA =====
            st.divider()
            st.subheader("📋 Gerar Resumo de Toda a Quinzena")
            
            if st.button("🔍 Gerar Resumo Completo", use_container_width=True, key="gerar_resumo"):
                st.session_state.show_resumo = True
            
            if st.session_state.get("show_resumo"):
                # Filtra TODA a quinzena (não só um entregador)
                ...
                for ent in sorted(df_quinzena['NOME_ENTREGADOR'].unique()):
                    df_ent = df_quinzena[df_quinzena['NOME_ENTREGADOR'] == ent]
                    ...
                    for idx, row in df_ent.iterrows():
                        ...
                        if tipo_lanc == "ENTREGA":
                            valor_base = (qtd * v_unit) + v_exc
                            total_cep_ent += valor_base
                            if cep:
                                mapa_cep_ent[cep] = mapa_cep_ent.get(cep, 0) + int(qtd)
                        
                        elif tipo_lanc == "COLETA":
                            valor_base = (qtd * v_unit) + v_exc
                            total_cep_ent += valor_base
                            if cep:
                                mapa_cep_col[cep] = mapa_cep_col.get(cep, 0) + int(qtd)
                        
                        elif tipo_lanc == "ROTA FECHADA":
                            rota_ent += valor_total
                        
                        elif tipo_lanc == "DESCONTO":
                            if v_desc == 0 and valor_total < 0:
                                desc_ent += abs(valor_total)
                        else:
                            if v_adic == 0 and valor_total > 0:
                                adic_ent += valor_total
                    
                    total_ent = total_cep_ent + adic_ent + rota_ent - desc_ent
                    cnpj_ent = df_ent['CNPJ'].iloc[0] if 'CNPJ' in df_ent.columns else ""
                    
                    resumo_entregadores.append({
                        'ENTREGADOR': ent,
                        'CNPJ': str(cnpj_ent).strip() if pd.notna(cnpj_ent) else "N/A",
                        'ENTREGAS': int(sum(mapa_cep_ent.values())),
                        'COLETAS': int(sum(mapa_cep_col.values())),
                        'TOTAL CEP': f"R$ {total_cep_ent:.2f}",
                        'ADICIONAIS': f"R$ {adic_ent:.2f}",
                        'DESCONTOS': f"R$ {desc_ent:.2f}",
                        'ROTA FECHADA': f"R$ {rota_ent:.2f}",
                        'TOTAL A RECEBER': f"R$ {total_ent:.2f}"
                    })
